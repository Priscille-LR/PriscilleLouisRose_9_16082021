...
describe('Given I am connected as an Admin', () => {
  ...
  describe('When I am on Dashboard page and I click on edit icon of a card', () => {
    ...
    //test lines 22-23
    test('Then, right form should be filled - new email (without ".")', () => {

      const billsNewMail = bills.map((bill) => { return {...bill, email: bill.email.replace('.', '') }} )

      const html = cards(billsNewMail)
      document.body.innerHTML = html
      ...
      Object.defineProperty(window, 'localStorage', { value: localStorageMock })
      const dashboard = new Dashboard({
        document, onNavigate, firestore, billsNewMail, localStorage: window.localStorage
      })

      const handleEditTicket = jest.fn((e) => dashboard.handleEditTicket(e, billsNewMail[0], billsNewMail))   
      const iconEdit = screen.getByTestId('open-bill47qAXb6fIm2zOKkLzMro')
      iconEdit.addEventListener('click', handleEditTicket)
      userEvent.click(iconEdit)
      expect(handleEditTicket).toHaveBeenCalled()
